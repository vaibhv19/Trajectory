  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-4 py-2">
        ...
        <div className="flex flex-col justify-between py-2 pr-6 border-b md:border-b-0 md:border-r border-border/30 pl-0 md:pl-6">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono font-bold uppercase tracking-wider text-muted-foreground">TOTAL SUBMITTED</span>
      ...
      <div className="flex flex-wrap gap-x-8 gap-y-2 py-4 border-y border-border/30 justify-start items-center text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
        {/* Funnel chart (65%) */}
        <div className="col-span-12 lg:col-span-8 py-2">
        ...
        {/* Agenda widget (35%) */}
        <div className="col-span-12 lg:col-span-4 py-2 pl-0 lg:pl-8 border-t lg:border-t-0 lg:border-l border-border/30">
          <div className="flex flex-col h-full justify-between">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-muted-foreground">Today's Agenda</h3>
              <span className="bg-muted text-muted-foreground border border-border/60 rounded-[4px] px-2 py-0.5 font-mono text-[10px]">
            ...
            <div className="flex-1 overflow-y-auto max-h-[240px] space-y-3 pr-1">
              {metrics.agenda.length === 0 ? (
                ...
                metrics.agenda.map((item) => (
                  <div 
                    key={item.id} 
                    className={`p-3 rounded-[4px] border flex flex-col justify-between hover:bg-muted/30 transition-all duration-150 ${
                      item.type === 'OA' ? 'border-status-oa-border bg-status-oa-bg/10' : 
                      item.type === 'INTERVIEW' ? 'border-status-interview-border bg-status-interview-bg/10' : 
                      'border-status-applied-border bg-status-applied-bg/10'
                    }`}
                  >
                    <div className="flex items-start justify-between">
                      ...
                      {item.link && (
                        <a 
                          ...
                          target="_blank" 
                          rel="noreferrer"
                          className="p-1 rounded-[4px] border border-border bg-card hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
                        >
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 pt-6 border-t border-border/30">
            ...
            {resumeData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={resumeData}>
                  <XAxis dataKey="name" stroke="currentColor" className="text-muted-foreground font-mono" fontSize={10} tickLine={false} />
                  <YAxis stroke="currentColor" className="text-muted-foreground font-mono" fontSize={10} tickLine={false} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))', borderRadius: '4px' }}
